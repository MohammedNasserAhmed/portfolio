...
moduleRegistry.set(4, function (module, exports, require) {
     ...
     */
    function handleError(error, context = '', type = ErrorTypes.COMPONENT) {
...
moduleRegistry.set(5, function (module, exports, require) {
    ...
    const { handleError: handleError, ErrorTypes: ErrorTypes, logInfo: logInfo } = loadModule(4);
    ...
    class SummaryComponent {
        ...
        getInlineIcon(name) {
            const iconMap = {
                ...
                retrieval:
                    "<svg viewBox='0 0 24 24' width='22' height='22' fill='none' stroke='currentColor' stroke-width='1.6'><path d='M3 7h18M3 12h18M3 17h18'/><path d='M8 5v4M12 10v4M16 15v4'/></svg>",
                pipeline:
                    "<svg viewBox='0 0 24 24' width='22' height='22' fill='none' stroke='currentColor' stroke-width='1.6'><rect x='3' y='3' width='7' height='7' rx='1'/><rect x='14' y='3' width='7' height='7' rx='1'/><rect x='3' y='14' width='7' height='7' rx='1'/><path d='M10 6h4M6.5 10v4M17.5 10v4M10 17h4'/></svg>"
            };
...
moduleRegistry.set(7, function (module, exports, require) {
    ...
    class ProjectsComponent {
        ...
        filterBySkills(activeFilters) {
            ...
            cards.forEach((card) => {
                const techBadges = Array.from(card.querySelectorAll('.tech-tags span')).map(
        ...
        init() {
            ...
            try {
                ...
                if (!this.pillsContainer) {
                    logWarn(
                        'Category pills container not found, some functionality may be limited'
                    );
                }
                ...
                throw handleError(
                    error,
                    'Failed to initialize SkillsComponent',
        ...
        updateCategoryPillStates(skills) {
            ...
            pills.forEach((pill) => {
                ...
                const skillsInCategory = skills
                    .filter((skill) => skill.category === category)
                    .map((skill) => skill.name);
                const allSelected = skillsInCategory.every((skill) =>
                    this.activeFilters.has(skill)
                );
    ...
    class PublicationsComponent {
        ...
        createBadgeHTML(publication) {
            ...
            if (publication.domain) {
                badges.push(
                    `<span class="pub-pill domain">${escapeHTML(publication.domain)}</span>`
                );
            }
        ...
        createMetaRow(publication) {
            const readingTime =
                publication.readingTime || this.estimateReadingTime(publication.description);
            let dateDisplay = '';
        ...
        buildControls(publications) {
            ...
            const types = Array.from(
                new Set(
                    publications.map((pub) => (pub.type || '').trim().toLowerCase()).filter(Boolean)
                )
            );
        ...
        applyFilters() {
            ...
            const activeTypeButton = this.controlsContainer?.querySelector(
            ...
            const filtered = this.currentPublications.filter((pub) => {
                const matchesType = activeType
                    ? (pub.type || '').toLowerCase() === activeType
                    : true;
        ...
        renderFiltered(publications) {
            ...
            const fillFeatured = publications.filter((pub) => !pub.featured).slice(0, fillCount);
            const featured = [...explicitFeatured, ...fillFeatured].slice(0, 2);

            const featuredIds = new Set(featured.map((pub) => pub.title + '|' + pub.published));
            const regular = publications.filter(
                (pub) => !featuredIds.has(pub.title + '|' + pub.published)
            );
            ...
            `;
                ...
                this.gridContainer.innerHTML = regular
                    .map((pub, i) => this.createCardHTML(pub, i))
                    .join('');
            }

...
class BuildSystem {
    ...
    async buildTemplates() {
        ...
        try {
            ...
            console.log(`   Generated: generated-index.html (${this.formatSize(generatedHTML.length)})`);
...
function loadModule(id) {
    ...
    processModuleContent(content, filePath, moduleMap) {
        ...
        processed = processed.replace(/export\s+(const|let|var|function|class)\s+(\w+)/g, 
            (match, type, name) => {
                return `${type} ${name}`;
            });
        
        // Handle imports
        processed = processed.replace(/import\s+(\w+)\s+from\s+['"`]([^'"`]+)['"`];?/g, 
    ...
    stripDevCode(code) {
        ...
        return code.replace(/\/\* DEV-OVERLAY-START \*\/[\s\S]*?\/\* DEV-OVERLAY-END \*\//g, 
        ...
        for (const match of importMatches) {
            ...
            try {
                if (await this.fileExists(resolvedPath)) {
                    ...
                    bundledCSS = bundledCSS.replace(fullMatch, `/* Imported from ${importPath} */\n${importedCSS}`);

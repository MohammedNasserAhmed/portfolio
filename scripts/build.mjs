...
if (prod) {
  ...
  const makeHash = (contents) => crypto.createHash('sha256').update(contents).digest('hex').slice(0, 10);
  ...
  pages.forEach((p) => {
    ...
    if (/meta name="asset-version"/i.test(html)) {
      html = html.replace(/<meta name="asset-version"[^>]*>/i, `<meta name="asset-version" content="${versionHash}">`);
    } else {
      html = html.replace(/<meta name="apple-mobile-web-app-status-bar-style"[^>]*>/i, (m) => `${m}\n        <meta name="asset-version" content="${versionHash}">`);
    }
  ...
  fs.writeFileSync(path.join(outDir, 'asset-manifest.json'), JSON.stringify(manifest, null, 2), 'utf8');
  ...
  console.log(`[build] Hashed bundles emitted JS=${hashedJS}${hashedCSS ? ' CSS=' + hashedCSS : ''} version=${versionHash}`);
